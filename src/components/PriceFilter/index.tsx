...
import { ItemsTypes, PaginationTypes, selectorPriceFilter, PriceFilterTypes } from '../../Data/Redux/ducks'
import { valueFilter1, valueFilter7} from '../../Shared/utils/index'

import { Filter } from './style'

import { dataFilter } from '../../Shared/utils/helper/filter'
            ...
            dataFilter.map(data => 
                <div key={data.min}>
                    ...
                    <label htmlFor={`filter${data.max}`}>
                        {data.min === valueFilter1 ? 'Até R$40' : data.max === valueFilter7 ? 'Acima de R$500' : `R$${data.min} A R$${data.max}`}
                    </label>
                </div>
            )
        )
